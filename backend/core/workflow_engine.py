...
class WorkflowAssistant:
        ...
        for part in parts:
            ...
            if any(x in part for x in ["convert", "pas", "transform", "llev", "cambi", "vuelv"]):
                ...
                if any(x in part for x in ["word", "doc"]):
                    ...
                    params["format"] = "jpg"
                elif any(x in part for x in ["png"]):
                    params["format"] = "png"
                elif any(x in part for x in ["pdf"]):
                    params["format"] = "pdf"
    ...
    def handle_user_intent(
        self, user_input: str, workflow: Optional[Workflow] = None
    ) -> Dict[str, Any]:
        """
        ...
        if not response_msg:
            ...
            action_names = [
                s.action.value.replace("_", " ") for s in workflow.steps[-new_step_count:]
            ]
            if len(action_names) > 1:
                response_msg = (
                    f"Genial, he configurado los pasos: {', '.join(action_names)}. ¿Ejecutamos?"
                )
            else:
                response_msg = (
                    f"Entendido, paso de '{action_names[0]}' añadido. ¿Algo más o ejecutamos?"
                )
