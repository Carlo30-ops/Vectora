...
from backend.core.workflow_engine import ActionType, StepStatus, Workflow, WorkflowStep
    ...
    def execute_workflow(
        ...
        for i, step in enumerate(workflow.steps):
            ...
            try:
                ...
                if not input_path:
                    raise ValueError(
                        f"No se pudo determinar el archivo de entrada para el paso {step.id}"
                    )
                ...
                step_result = self._execute_step_action(
                    ...
                    (
                        ...
                        else [str(p) for p in input_path]
                    ),
                    str(output_path),
                )
                ...
                results.append({"step_id": step.id, "status": "success", "output": final_output})
