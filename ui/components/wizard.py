...
from .ui_helpers import AnimatedCard, FadingStackedWidget, IconHelper
...
WIZARD_QUESTIONS = {
    "start": {
        ...
        "options": [
            {
                "text": "Combinar varios archivos",
                "desc": "Unir múltiples PDFs en uno solo",
                "icon": "merge",
                "result": "merge",
            },
            {
                "text": "Separar páginas",
                "desc": "Extraer o dividir un documento",
                "icon": "split",
                "result": "split",
            },
            {
            ...
            },
            {
                ...
                "desc": "PDF a Word, Imágenes, etc.",
                "icon": "convert",
                "next": "convert",
            },
            {
                "text": "Proteger",
                "desc": "Encriptar con contraseña",
                "icon": "security",
                "result": "security",
            },
            {
                "text": "Reconocer texto (OCR)",
                "desc": "Hacer buscable un escaneado",
                "icon": "ocr",
                "result": "ocr",
            },
        ],
    ...
    "convert": {
        ...
        "options": [
            ...
            {
                "text": "PDF a Imágenes",
                "desc": "Extraer páginas como fotos",
                "icon": "image",
                "result": "convert",
                "mode": 1,
            },  # 1 = PDF->Img
...
class Wizard(QWidget):
    ...
    def show_question(self, q_id):
        ...
        if q_id not in WIZARD_QUESTIONS:
            return
        ...
        if q_id != "start":
            ...
            back_btn.setStyleSheet(
                "border: none; text-align: left; font-size: 14px; color: #6b7280;"
            )
    ...
    def update_theme(self, theme_name=None):
        ...
        for lbl in current.findChildren(QLabel, "wizardTitle"):
            lbl.setStyleSheet(
                f"font-size: 32px; font-weight: 800; color: {txt_p}; font-family: 'Inter'; letter-spacing: -1px;"
            )
        ...
        for card in current.findChildren(AnimatedCard):
            ...
            if t_lbl:
                t_lbl.setStyleSheet(
                    f"font-size: 18px; font-weight: 700; color: {txt_p}; border: none; background: transparent; font-family: 'Inter';"
                )

            d_lbl = card.findChild(QLabel, "optionDesc")
            if d_lbl:
                d_lbl.setStyleSheet(
                    f"font-size: 14px; color: {txt_s}; border: none; background: transparent; font-family: 'Inter';"
                )
