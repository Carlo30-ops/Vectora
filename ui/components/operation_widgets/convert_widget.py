...
class ConvertWidget(BaseOperationWidget):
    ...
    def start_processing(self):
        ...
        if mode_idx == 0: # PDF -> Word
            ...
            out = str(settings.OUTPUT_DIR / f"{os.path.basename(self.current_file)[:-4]}.docx")
            kwargs = {'input_path': self.current_file, 'output_path': FileHandler.ensure_unique_filename(out)}
            
        elif mode_idx == 1: # PDF -> Images
            if not self.current_file: return self.show_error("Selecciona un archivo")
            worker_mode = 'pdf_to_images'
            out_dir = str(settings.OUTPUT_DIR / "images")
            kwargs = {
            ...
            out = str(settings.OUTPUT_DIR / "images_combined.pdf")
            kwargs = {'image_paths': self.image_files, 'output_path': FileHandler.ensure_unique_filename(out)}
            
        elif mode_idx == 3: # Word -> PDF
            if not self.current_file: return self.show_error("Selecciona un archivo")
            worker_mode = 'word_to_pdf'
            out = str(settings.OUTPUT_DIR / f"{os.path.basename(self.current_file)[:-5]}.pdf")
            kwargs = {'input_path': self.current_file, 'output_path': FileHandler.ensure_unique_filename(out)}
