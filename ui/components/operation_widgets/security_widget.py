...
class SecurityWidget(BaseOperationWidget):
    ...
    def start_processing(self):
        ...
        if mode == 'encrypt':
            ...
            output = str(settings.OUTPUT_DIR / f"protected_{FileHandler.get_filename(self.current_file)}")
            kwargs = {
                'input_path': self.current_file,
                'output_path': FileHandler.ensure_unique_filename(output),
                'password': pwd,
                'permissions': {
            ...
            output = str(settings.OUTPUT_DIR / f"unlocked_{FileHandler.get_filename(self.current_file)}")
            kwargs = {
                'input_path': self.current_file,
                'output_path': FileHandler.ensure_unique_filename(output),
                'password': pwd
