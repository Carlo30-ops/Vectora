...
class OCRWidget(BaseOperationWidget):
    ...
    def start_processing(self):
        ...
        output = str(settings.OUTPUT_DIR / f"ocr_{FileHandler.get_filename(self.current_file)}")
        output = FileHandler.ensure_unique_filename(output)
        
        self.set_processing_state(True)
        self.update_progress(0, "Iniciando motor OCR...")
        
        self.worker = OCRWorker(self.current_file, output, lang_code, dpi)
