...
class DragDropMixin:
    ...
    def enable_drag_drop(
        self, accepted_extensions: Optional[List[str]] = None, multiple: bool = False
    ):
        """
        ...
        if is_dragging:
            ...
            if new_style == current:
                ...
                new_style = new_style.replace(
                    "background-color: {{HOVER}}", "background-color: {{ACCENT}}20"
                )
...
def _set_drag_style(frame: QFrame, is_dragging: bool):
    ...
    if is_dragging:
        ...
        new_style = current.replace("border: 2px dashed {{BORDER}}", "border: 2px solid {{ACCENT}}")
        if new_style == current:
            new_style = (
                current
                + "\nborder: 2px solid {{ACCENT}} !important; background-color: {{ACCENT}}20;"
